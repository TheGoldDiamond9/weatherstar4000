    ...
    $.getJSON(url, function(data) {
      ...
      try {
        ...
        for (var i = 0; i < weatherData.extendedForecast.days.length; i++, ii++, dpi = dpi + 2) {
          weatherData.extendedForecast.days[i].cond = data.daypart[0].wxPhraseLong[dpi].replace('Scattered ', "Sct'd ").replace('Thunderstorms',"Thunder-\nstorms").replace('Thundershowers',"Thunder-\nshowers")
